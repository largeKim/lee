...
if (window.jQuery)(function ($) {
	...
	$.fn.MultiFile = function (options) {
		...
		if (typeof arguments[0] == 'string') {
			...
			// Invoke API method handler (and return whatever it wants to
			// return)
		...
		this
			...
			.each(function () {
				...
				// BUG 1251 FIX:
				// http://plugins.jquery.com/project/comments/add/1251
				// variable group_count would repeat itself on multiple calls to
				// the plugin.
				// this would cause a conflict with multiple elements
				// changes scope of variable to global so id will be unique over
				// n calls
				...
				var o = $.extend({},
					...
					options || {}, ($.metadata ? MultiFile.E.metadata() : ($.meta ? MultiFile.E.data() : null)) || {}, /*
																														 * metadata
																														 * options
																														 */ {} /* internals */
				);
				...
				// size options are accepted in kylobytes, so multiple them by
				// 1024
				...
				$.extend(MultiFile, {
					...
					slaves: [],
					files: [],
					instanceKey: MultiFile.e.id || 'MultiFile' + String(group_count), // Instance
																						// Key?
				...
				// http://smyck.de/2006/08/11/javascript-dynamic-regular-expresions/
				...
				MultiFile.wrapID = MultiFile.instanceKey + '_wrap'; // Wrapper
																	// ID?
				//console.log(MultiFile.wrapID);
				MultiFile.E.wrap('<div class="MultiFile-wrap" id="' + MultiFile.wrapID + '"></div>');
				MultiFile.wrapper = $('#' + MultiFile.wrapID + '');

				// ===

				// MultiFile MUST have a name - default: file1[], file2[],
				// file3[]
				...
				if (!MultiFile.list) {
					...
					// * OPERA BUG: NO_MODIFICATION_ALLOWED_ERR ('list' is a
					// read-only property)
					// this change allows us to keep the files in the order they
					// were selected
				...
				MultiFile.addSlave = function (slave, slave_count) {
					// if(window.console)
					// console.log('MultiFile.addSlave',slave_count);
					...
					// Define element's ID and name (upload components need
					// this!)
					// slave.id = slave.id || MultiFile.generateID(slave_count);
					if (slave_count > 0) slave.id = MultiFile.generateID(slave_count);
					// FIX for:
					// http://code.google.com/p/jquery-multifile-plugin/issues/detail?id=23

					// 2008-Apr-29: New customizable naming convention (see url
					// below)
					...
					slave.change(function (a, b, c) {
						// if(window.console)
						// console.log('MultiFile.slave.change',slave_count);
						// if(window.console)
						// console.log('MultiFile.slave.change',this.files);

						// Lose focus to stop IE7 firing onchange again
						$(this).blur();

						// # NEW 2014-04-14 - accept multiple file selection,
						// HTML5
						var e = this,
						...
						// recap
						// console.log('START '+ prevs.length + ' files @ '+
						// sl(total_size) +'.', prevs);
						...
						$.each(files, function (i, file) {
							...
							;
							/*
							 * console.log(v); console.log(s); console.log(p);
							 */
						...
						// add up total for all files selected (existing and
						// new)
						...
						var newEle = $(MultiFile.clone).clone(); // Copy
																	// parent
																	// attributes
																	// - Thanks
																	// to Jonas
						...
						if (ERROR.length > 0) {
							...
							// 2007-06-24: BUG FIX - Thanks to Adrian Wr�bel
							// <adrian [dot] wrobel [at] gmail.com>
							// Ditch the trouble maker and add a fresh new
							// element
							MultiFile.n--;
							...
							// recap
							// console.log('NOW '+ files.length + ' files @ '+
							// sl(total_size) +'.', files);
				...
				MultiFile.addToList = function (slave, slave_count, files) {
					...
					$.each(files, function (i, file) {
						...
						names[names.length]=
							(
								'<span name="test2" class="MultiFile-title" title="' + MultiFile.STRING.selected + '">'
									+ MultiFile.STRING.file +
								'</span><div class="del">'+fSize+'</div>'
								/*
								 * '<span class="MultiFile-title" title="' +
								 * MultiFile.STRING.selected + '">' +
								 * MultiFile.STRING.file + '</span>'
								 */
							)		
...
// /////////////////////////////////////////////////////////
				// $.each(files, function (i, file) {
						...
						var
							...
							b = $('<a class="MultiFile-remove" href="#' +MultiFile.wrapID + '">' + MultiFile.STRING.remove + '</a>').click(function () {
									...
									// Remove element, remove label, point to
									// current
									MultiFile.slaves[slave_count] = null;
									$(slave).remove();
									$(this).parent().remove();
								
									// Show most current element again (move
									// into view) and clear selection
									...
									// rebuild array with the files that are
									// left.
									var newfs = [];
...
// *************888
	...
	$.extend($.fn.MultiFile, {
		...
		/**
		 * This method exposes the plugin's sum of the sizes of all files
		 * selected
		 * 
		...
		/**
		 * This method exposes the plugin's tally of how many files have been
		 * selected
		 * 
		...
		/**
		 * This utility makes it easy to disable all 'empty' file elements in
		 * the document before submitting a form. It marks the affected elements
		 * so they can be easily re-enabled after the form submission or
		 * validation.
		 * 
		 * Returns a jQuery collection of all affected elements.
		 * 
		 ...
		 * @param String
		 *            class (optional) A string specifying a class to be applied
		 *            to all affected elements - Default: 'mfD'.
		 */
		...
		/**
		 * This method re-enables 'empty' file elements that were disabled (and
		 * marked) with the $.fn.MultiFile.disableEmpty method.
		 * 
		 * Returns a jQuery collection of all affected elements.
		 * 
		 * @name reEnableEmpty
		 ...
		 * @param String
		 *            klass (optional) A string specifying the class that was
		 *            used to mark affected elements - Default: 'mfD'.
		 */
		...
		/**
		 * This method will intercept other jQuery plugins and disable empty
		 * file input elements prior to form submission
		 * 
		 * 
		 * @name intercept
		 * @cat Plugins/MultiFile
		 ...
		 * @param Array
		 *            methods (optional) Array of method names to be intercepted
		 */
		intercepted: {},
		intercept: function (methods, context, args) {
			...
			for (var i = 0; i < methods.length; i++) {
				...
				if (method)(function (method) { // make sure that method is
												// ISOLATED for the interception
					$.fn.MultiFile.intercepted[method] = $.fn[method] || function () {};
					$.fn[method] = function () {
						$.fn.MultiFile.disableEmpty();
						value = $.fn.MultiFile.intercepted[method].apply(this, arguments);
						// SEE
						// http://code.google.com/p/jquery-multifile-plugin/issues/detail?id=27
						setTimeout(function () {
							$.fn.MultiFile.reEnableEmpty()
						}, 1000);
						return value;
					}; // interception
				})(method); // MAKE SURE THAT method IS ISOLATED for the
							// interception
			}; // for each method
	...
	/*
	 * ### Default Settings ### eg.: You can override default control like this:
	...
	$.fn.MultiFile.options = { // $.extend($.fn.MultiFile, { options: {
		...
		// name of methods that should be automcatically intercepted so the
		// plugin can disable
		// extra file elements that are empty before execution and automatically
	...
	/*
	 * ### Additional Methods ### Required functionality outside the plugin's
	 * scope
	 */

	// Native input reset method - because this alone doesn't always work:
	// $(element).val('').attr('value', '')[0].value = '';
	...
	/*
	 * ### Default implementation ### The plugin will attach itself to file
	 * inputs with the class 'multi' when the page loads
	 */
	$(function () {
